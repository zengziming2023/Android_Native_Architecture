...
object TaskSortUtil {
    ...
    fun sortAppStartTask(
        taskList: ArrayList<out Task>,//原始任务列表
        mTaskMap: HashMap<Class<out Task>, Task>,//按class存储每个任务，方便查找
        mTaskChildMap: HashMap<Class<out Task>, HashSet<Class<out Task>>>//保存任务以及子任务
    ): ArrayList<Task> {
        //最终排序列表
        val sortTaskList: ArrayList<Task> = ArrayList()
        //存储任务包含的父任务数
        val taskIntMap: HashMap<Class<out Task>, Int> = HashMap()
        val deque: Deque<Class<out Task>> = ArrayDeque()
        //首先分别取出数据，并入栈入度为0的数据
        //把孩子都加进去
